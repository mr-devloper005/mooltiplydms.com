...
const cardBase = 'group block rounded-2xl border border-white/[0.06] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1.5 hover:border-white/[0.12] hover:shadow-[0_20px_60px_rgba(0,0,0,0.5)]'
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-white/[0.04]">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,var(--tk-glow),transparent_70%)]" />
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-20 sm:py-28 lg:px-8">
            <div className="flex items-center gap-3 text-[11px] font-medium uppercase tracking-[0.3em] text-[var(--tk-accent)]">
              <span>{theme.kicker}</span>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-accent)] opacity-50" />
              <span className="text-[var(--tk-muted)]">{label}</span>
            </div>
            <h1 className="editable-display mt-6 max-w-3xl text-balance text-[2.5rem] font-bold leading-[1.06] tracking-[-0.03em] sm:text-5xl lg:text-6xl">
            ...
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-1.5 text-xs font-medium text-[var(--tk-muted)]">{chip}</span>
                ))}
              </div>
            ) : null}
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-full border border-white/[0.08] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-11 items-center rounded-full bg-white px-5 text-sm font-bold text-[#080c18] transition hover:bg-white/90">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
          {posts.length ? (
            ...
            <div className="mx-auto max-w-xl rounded-2xl border border-dashed border-white/[0.08] bg-[var(--tk-surface)] px-8 py-16 text-center">
          ...
          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-white/[0.08] px-5 py-2.5 font-medium transition hover:bg-white/5">Previous</Link> : null}
              <span className="rounded-full border border-white/[0.08] bg-[var(--tk-surface)] px-5 py-2.5 font-medium text-[var(--tk-muted)]">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-white/[0.08] px-5 py-2.5 font-medium transition hover:bg-white/5">Next</Link> : null}
            </nav>
          ) : null}
        </section>
      </main>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/[0.06] bg-[var(--tk-raised)]">
...
function ClassifiedArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6`}>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-white/[0.06] pt-4 text-xs font-medium text-[var(--tk-muted)]">
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-2xl border border-white/[0.06] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-1 hover:border-white/[0.12]">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-5">
          <h2 className="editable-display line-clamp-2 text-lg font-bold leading-snug text-white">{post.title}</h2>
          <span className="mt-2 inline-flex items-center gap-1.5 text-xs font-medium text-white/60">View <ArrowUpRight className="h-3.5 w-3.5" /></span>
        </div>
      </div>
    </Link>
  )
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6`}>
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-white/[0.08] px-3 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">{category}</span>
      </div>
      <h2 className="editable-display mt-6 text-lg font-bold leading-snug tracking-[-0.02em]">{post.title}</h2>
      <p className="mt-3 line-clamp-3 flex-1 text-sm leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col items-center p-7 text-center`}>
      <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full border border-white/[0.06] bg-[var(--tk-raised)]">
